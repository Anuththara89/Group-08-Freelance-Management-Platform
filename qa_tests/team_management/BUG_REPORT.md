...
**Run result:** 33 passed · **14 failed** · 0 skipped · 0 soft passes
...
|---|---|---|---|
| Core CRUD | 19 | 19 | 0 |
| Edge Cases & Validation | 17 | 8 | 9 |
| Relational Integrity | 5 | 4 | 1 |
| Security | 6 | 2 | 4 |
| **TOTAL** | **47** | **33** | **14** |
...
## BUG-07 · MEDIUM · Negative Salary Accepted Without Validation
**Test failed:** `test_create_with_negative_salary_is_rejected`
**Endpoint:** `POST /api/freelancer/create`

### Description
The API accepts a negative salary value (e.g., `-1000.00`) and creates the freelancer with status `201`. There is no application-layer or database-level validation to prevent this. A negative salary is invalid business logic — no freelancer should be paid a negative amount.

### Observed
- `POST` with `{"salary": -1000.00}` → **201 Created**
- The freelancer is stored in the database with `salary = -1000.00`.

### Impact
- Corrupts financial data — payroll calculations, invoices, and finance reports will produce incorrect results.
- No guardrail exists at any layer (DTO, service, or DB constraint).

### Fix
Add a `@Positive` or `@Min(0)` Bean Validation annotation to `FreelancerDTO`:
...
And ensure `@Valid` is on the controller method (same fix as BUG-04).
...
| Negative salary | Must be rejected | ❌ FAIL (BUG-07) |
| Zero salary | Accepted or rejected gracefully | ✅ PASS |
...
| BUG-07 | **MEDIUM** | Negative salary accepted without validation | Low (~15min) |

---

*Report generated by automated QA suite — `qa_tests/team_management/` · Run time: 74s · 47 tests · 0 skips · 0 soft passes*
